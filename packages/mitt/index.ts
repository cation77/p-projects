...
export type WildcardHandlerList<T = Record<string, unknown>> = Array<
  WildcardHandler<T>
>
...
export default function mitt<Events extends Record<EventType, unknown>>(
  ...
  return {
    ...
    emit<Key extends keyof Events>(type: Key, evt?: Events[Key]) {
      let handlers: GenericEventHandler[] | undefined = all!.get(type)
      if (handlers) {
      ...
      handlers = all!.get('*')
      if (handlers) {
        ;(handlers as WildcardHandlerList<Events>).slice().map((handler) => {
          handler(type, evt!)
        })
      }
    }
